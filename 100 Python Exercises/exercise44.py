...
This is like the previous exercise with the difference that we're using a
different slicing step of 3 here. Another difference is that we are adding one
white spaces to ascii_lowercase  so that we get a string of 27 items as opposed
to 26 that ascii_lowercase  provides. By slicingascii_lowercase we would get
two slices with a length of 9 and one slice with a length of 8.
...
three slices each with a length of 9. That means we get 9 iterations and not
8 as we would get with the original ascii_lowercase  string. The 9th iteration
